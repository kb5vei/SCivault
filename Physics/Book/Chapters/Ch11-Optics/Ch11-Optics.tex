...
\end{mdframed}
	...
	\end{mdframed}	
		...
		Like convex lenses, Concave mirrors are converging optical devices - that is, they focus parallel rays of light to a point.  Unlike lenses, mirrors only have one focal point, since light can only interact with the mirror from one direction.  
		...
		\begin{center}
			\begin{figure}[H]
				\begin{center}
					...
					\draw [thick, fill=white] (-.2,-.9) 	arc[start angle=-35, end angle=35,radius=1.5];
					...
					\draw[red,reverse directed] (-5,-.8) -- (0,.6) ;
					\draw[red,reverse directed] (-5,.8) -- (0,-.6) ;			
					\draw[red,reverse directed] (-5,-.4) -- (0,.3) ;			
					\draw[red,reverse directed] (-5,.4) -- (0,-.3) ;	
					\draw[red,reverse directed] (-5,0) -- (0,0) ;	
					
					
					\end{tikzpicture}
					\caption{A concave mirror focuses light to a point.}
					\label{fig:concavemirror}
				\end{center}
			\end{figure}
		\end{center}
	...
	\section{Physical Optics}
		...
		\begin{figure}[h]		
		\begin{center}
			\includegraphics[height=2in]{./Chapters/Ch11-Optics/thinfilm.png}
		\end{center}
